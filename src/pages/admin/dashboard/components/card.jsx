...
const API_URL = "https://apiricoton.cartavirtual.shop/api/recursos-empresa";
const ID_RECURSO = 1;
...
const UploadImagesCard = () => {
  ...
  const fetchRecursos = async () => {
    try {
      ...
      if (data.logo_url)
        setLogo(`https://apiricoton.cartavirtual.shop/${data.logo_url}?t=${Date.now()}`);
      if (data.portada_url)
        setBanner(`https://apiricoton.cartavirtual.shop/${data.portada_url}?t=${Date.now()}`);
    } catch (error) {
